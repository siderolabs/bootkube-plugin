...
ENV CGO_ENABLED 0
WORKDIR /tmp
RUN curl -sfL https://install.goreleaser.com/github.com/golangci/golangci-lint.sh | bash -s -- -b /go/bin v1.24.0
RUN cd $(mktemp -d) \
  && go mod init tmp \
...
FROM base AS binary
RUN --mount=type=cache,target=/root/.cache/go-build GOOS=linux go build -ldflags "-s -w" -buildmode=plugin -o /render.so ./cmd

FROM scratch AS container
COPY --from=binary /render.so render.so/
ENTRYPOINT [ "/render.so" ]
